...
namespace TqkLibrary.Avalonia.ToolKit.Collections.AsyncCollections
{
    public class AsyncCollection<TData, TViewModel> : 
        IList<TViewModel>, 
        IList, 
        ...
        where TViewModel : IViewModel<TData>
    {
        ...
        protected virtual void FireCollectionReset()
        {
            ...
            foreach (int keyPage in _pages.Keys)
            {
                ...
                {
                    _pages[keyPage].Items[keyIndex].ViewModel = default;
        ...
        {
            _asyncData = asyncData ?? throw new ArgumentNullException(nameof(asyncData));
        ...
        class PageData
        {
            ...
            public PageData(int pageSize)
            {
                ...
                {
                    Items[i] = new ItemData() { ViewModel = default };
                }
            }
        ...
        class DictPageData : Dictionary<int, PageData>
        {
            public IEnumerable<TViewModel?> ViewModels
            {
                get
                {
                    foreach (PageData pageData in Values.ToList())
                    {
        ...
        {
            using CancellationTokenSource timeout = new(FetchTimeout);
            int count = await _asyncData.CountAsync(timeout.Token);
            if (count != _count || force)
            {
                _count = count;
                _ = Dispatcher.InvokeAsync(FireCollectionReset);
            }
        }
